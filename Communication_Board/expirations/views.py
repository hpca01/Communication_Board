...
class DrugListView(ListView):
    ...
    query_all = Drug.objects.all()

    def get(self, request):
        if self.request.is_ajax():
            text_to_search = self.request.GET.get('searchText')
            #print(text_to_search)
            return render(request, "expirations/list.html", {'drugs':self.query_all.filter(name__contains = text_to_search).order_by('name')})
        else:
            return render(request, "expirations/drug_list.html", {'drugs':self.query_all.order_by('name')})
...
class CreateExpirationView(AjaxableResponseMixin, CreateView):
    ...
    def form_invalid(self, form):
        ...
        if form.is_ajax():
            ...
            return JsonResponse({'message':'Failed'}, encoder=DjangoJSONEncoder, safe=True, json_dumps_params=None)
